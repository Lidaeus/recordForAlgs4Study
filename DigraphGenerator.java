...
public class DigraphGenerator {
    ...
    public static Digraph eulerianCycle(int V, int E) {
        ...
        if (V <= 0)
            throw new IllegalArgumentException("An Eulerian cycle must have at lease one vertex");
        Digraph G = new Digraph(V);
    ...
    public static Digraph strong(int V, int E, int c) {
        if (c >= V || c <= 0)
            throw new IllegalArgumentException("Number of components must be between 1 and V-1");
        if (E <= 2 * (V - c))
        ...
        for (int i = 0; i < c; i++) {
            ...
            for (int v = 0; v < count - 1; v++) {
                ...
                Edge e = new Edge(w, v);
                if (!set.contains(e) && v != w && lable[v] <= lable[w]) {
                    set.add(e);
                    G.addEdge(v, w);
                }
            }
