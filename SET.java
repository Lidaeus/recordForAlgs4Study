...
public class SET<Key extends Comparable<Key>> implements Iterable<Key> {
    ...
    public Key min() {
        if (isEmpty()) throw new NoSuchElementException("caller min() with empty set");
        return set.first();
    }
    ...
    public SET<Key> intersects(SET<Key> that) {
        if (that == null) throw new IllegalArgumentException("called intersects() with a null key");
        SET<Key> c = new SET<Key>();
